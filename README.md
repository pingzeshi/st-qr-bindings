...
2. [st-api-wrapper](https://github.com/Lianues/st-api-wrapper)，安装路径应为：

   `public/scripts/extensions/third-party/st-api-wrapper`

本扩展通过 `window.ST_API.worldBook.*` 和 `window.ST_API.preset.*` 读取世界书与预设列表，因此没有安装 st-api-wrapper 时无法正常使用。
...
```text
public/scripts/extensions/third-party/qr-bindings
```
